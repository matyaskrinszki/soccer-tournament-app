...
function insertSampleData(db) {
    ...
    // Teams by league with fixed ids (6 teams per league)
    const teamsByLeague = {
        'premier-league': [
        ...
        'ligue-1': [
            { id: 25, name: 'Paris Saint-Germain' },
            { id: 26, name: 'Marseille' },
            { id: 27, name: 'Lyon' },
            { id: 28, name: 'Nice' },
            { id: 29, name: 'Monaco' },
            { id: 30, name: 'Lille' },
    ...
    Object.entries(teamsByLeague).forEach(([leagueId, teams]) => {
        teams.forEach(team => {
            db.run(
                "INSERT INTO teams (id, name, league_id, matches, wins, draws, losses, points) VALUES (?, ?, ?, 0, 0, 0, 0, 0)",
    ...
    let playerId = 1;
    const positions = ['GK', 'DF', 'DF', 'MF', 'MF', 'FW', 'FW'];
